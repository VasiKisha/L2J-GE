		...
		if ((qs != null) && qs.isStarted())
		{
			switch (npc.getId())
			{
				...
				case CRIMSON_WEREWOLF:
				{
					...
					{
						npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_GENERAL, "Cowardly guy!"));
						npc.deleteMe();
					}
		...
		if ((qs != null) && qs.isStarted() && LocationUtil.checkIfInRange(Config.ALT_PARTY_RANGE, npc, killer, true))
		{
			switch (npc.getId())
			{
				...
				case ARURAUNE:
				{
					...
					{
						npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_GENERAL, "The concealed truth will always be revealed...!"));
