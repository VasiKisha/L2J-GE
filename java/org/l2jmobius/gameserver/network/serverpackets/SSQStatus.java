		...
		switch (_page)
		{
			case 1:
			{
				...
				switch (currentPeriod)
				{
					...
					{
						buffer.writeInt(SystemMessageId.THIS_IS_A_QUEST_EVENT_PERIOD.getId());
						break;
					}
					case SevenSigns.PERIOD_COMP_RESULTS:
					{
						buffer.writeInt(SystemMessageId.THIS_IS_A_PERIOD_OF_CALCULATING_STATISTICS_IN_THE_SERVER.getId());
						break;
					}
							...
							switch (winningCabal)
							{
								...
								case SevenSigns.CABAL_DAWN:
								{
									if (dawnPercent >= 35)
									{
										...
										buffer.writeInt(SystemMessageId.ALTHOUGH_THE_SEAL_WAS_NOT_OWNED_SINCE_35_OR_MORE_PEOPLE_HAVE_VOTED.getId());
									}
									else
									{
										buffer.writeByte(SevenSigns.CABAL_NULL);
						...
						case SevenSigns.CABAL_DAWN:
						{
							switch (winningCabal)
							{
								case SevenSigns.CABAL_NULL:
								{
									...
									{
										buffer.writeByte(SevenSigns.CABAL_DAWN);
										buffer.writeInt(SystemMessageId.SINCE_THE_SEAL_WAS_OWNED_DURING_THE_PREVIOUS_PERIOD_AND_10_OR_MORE_PEOPLE_HAVE_VOTED.getId());
									}
									else
									{
										buffer.writeByte(SevenSigns.CABAL_NULL);
								...
								case SevenSigns.CABAL_DAWN:
								{
									...
									{
										buffer.writeByte(SevenSigns.CABAL_NULL);
										buffer.writeInt(SystemMessageId.ALTHOUGH_THE_SEAL_WAS_OWNED_DURING_THE_PREVIOUS_PERIOD_BECAUSE_LESS_THAN_10_OF_PEOPLE_HAVE_VOTED.getId());
									}
									break;
								}
								case SevenSigns.CABAL_DUSK:
